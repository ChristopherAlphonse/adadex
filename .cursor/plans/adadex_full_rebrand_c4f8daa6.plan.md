...
todos:
  ...
  - id: core-domain
    ...
    status: completed
  - id: api-routes
    content: Rename API routes deck/tentacles and /api/tentacles/git; update deck/git handlers
    status: completed
  - id: web-ui
...
| ----------------------------- | --------------------------------------------------------------------------------------------------- |
| Octogent                      | **Adadex**                                                                                          |
| tentacle / tentacles (domain) | **coordination** / **coordinations**                                                                |
| `tentacleId`                  | `coordinationId`                                                                                    |
| `.octogent/`                  | `.adadex/`                                                                                          |
| `~/.octogent/projects/`       | `~/.adadex/projects/`                                                                               |
| `tentacles.json` (registry)   | `coordinations.json`                                                                                |
| `.octogent/tentacles/<id>/`   | `.adadex/coordinations/<id>/`                                                                       |
| `@octogent/*`                 | `@adadex/*`                                                                                         |
...
- `/api/deck/tentacles` → `/api/deck/coordinations` (and all nested `/api/deck/tentacles/:id/...`)
- `/api/tentacles/:tentacleId/git/...` → `/api/coordinations/:coordinationId/git/...`
...
- Rename types such as `DeckTentacleSummary`, `DeckTentacleStatus` → `DeckCoordinationSummary`, `DeckCoordinationStatus` in [packages/core/src/domain/deck.ts](packages/core/src/domain/deck.ts) and propagate through core tests ([packages/core/tests/buildTerminalList.test.ts](packages/core/tests/buildTerminalList.test.ts), etc.).
- Rename `tentacleId` fields in JSON shapes to `coordinationId` where they represent deck/coordination rows (verify terminal vs coordination IDs — some code paths conflate names; align with existing semantics).

## API and filesystem

- Centralize path constants (e.g. `.adadex`, `coordinations`, `coordinations.json`) in one module if not already, then replace string literals across [apps/api](apps/api) (notably [terminalRoutes.ts](apps/api/src/createApiServer/terminalRoutes.ts), [deckRoutes.ts](apps/api/src/createApiServer/deckRoutes.ts), [gitRoutes.ts](apps/api/src/createApiServer/gitRoutes.ts), [readDeckTentacles.ts](apps/api/src/deck/readDeckTentacles.ts) → renamed file).
- **Migration (required for full rename):** On API/bootstrap, if legacy paths exist and new paths do not, atomically migrate:
  - `.octogent` → `.adadex`
  - `state/tentacles.json` → `state/coordinations.json`
  - `tentacles/` subfolder → `coordinations/` under project dir
...
- Rename components and hooks: e.g. `TentaclePod.tsx` → `CoordinationPod.tsx`, `AddTentacleForm` → `AddCoordinationForm`, `useTentacleGitLifecycle` → `useCoordinationGitLifecycle`, [CanvasTentaclePanel](apps/web/src/components/canvas/CanvasTentaclePanel.tsx), etc.
- Update CSS classes / testids only where they encode `tentacle` if those are part of the public contract; otherwise keep stable selectors if tests depend on them (prefer updating tests to new names for consistency).

**Octopus visuals:** Types like `DeckOctopusAppearance` / `octopus` fields are **not** the word “tentacle” but reinforce the old metaphor. **Recommendation:** keep them for this PR to limit scope, or rename to neutral `mascot` / `avatar` only where you already touch serialization — document choice in PR. Slogan and copy will shift to execution/coordination language regardless.

## Prompts and sync

- Rename prompt files that embed `tentacle` in the filename ([prompts/tentacle-planner.md](prompts/tentacle-planner.md), etc.) and update [apps/api](apps/api) prompt resolution / any hardcoded names.
...
- Systematic pass: [README.md](README.md), [AGENTS.md](AGENTS.md), [CONTRIBUTING.md](CONTRIBUTING.md), [docs/](docs/) (including rename [docs/concepts/tentacles.md](docs/concepts/tentacles.md) → `coordination.md` and fix cross-links in [docs/index.md](docs/index.md)).
...
This is a **breaking change** for any automation, scripts, or external clients that call `/api/deck/tentacles` or assume `.octogent`. Document breaking changes in README “Upgrading” section.
