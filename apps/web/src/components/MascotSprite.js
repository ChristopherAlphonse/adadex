...
const getMotionOffset = (animation, frame, unit) => {
    if (animation === "idle") {
...
const getExpressionAlpha = (expression) => {
    switch (expression) {
        case "happy":
            return 0.98;
        case "sleepy":
...
const drawArmStroke = (ctx, sx, sy, cpx, cpy, ex, ey, lineWidth, outlineWidth, fillColor, strokeAlpha) => {
    ctx.save();
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    ctx.beginPath();
...
const resolveHairColor = (hairColor) => hairColor && hairColor.trim().length > 0 ? hairColor : "#5c4033";
const drawAccessoryHair = (ctx, accessory, hairHex, headCy, headRx, headRy, unit) => {
    ...
    if (accessory === "none") {
        ...
        ctx.bezierCurveTo(-headRx * 0.66, crownY - unit * 0.55, -headRx * 0.08, crownY - unit * 0.9, headRx * 0.72, crownY - unit * 0.18);
        ctx.bezierCurveTo(headRx * 0.42, crownY + unit * 0.32, headRx * 0.02, crownY + unit * 0.42, -headRx * 0.3, crownY + unit * 0.24);
        ctx.bezierCurveTo(-headRx * 0.36, crownY + unit * 0.78, -headRx * 0.58, sideY + unit * 0.12, -headRx * 0.82, sideY);
        ctx.fill();
        ctx.stroke();
    ...
    switch (accessory) {
        case "long": {
            for (let i = 0; i < 5; i++) {
                ...
                ctx.beginPath();
                ctx.moveTo(sx, sy);
                ctx.quadraticCurveTo(sx - unit * (1.8 + t * 0.4), sy + unit * (1.2 + t * 0.8), sx - unit * (2.4 + t * 0.3), sy + unit * (3.5 + t * 0.5));
                ctx.stroke();
            }
            break;
        }
        ...
        case "side-sweep": {
            ...
            ctx.bezierCurveTo(headRx * 0.85, headCy - headRy * 1.22, headRx * 1.75, headCy - headRy * 0.82, headRx * 1.85, headCy - headRy * 0.32);
            ctx.bezierCurveTo(headRx * 1.35, headCy + headRy * 0.08, headRx * 0.45, headCy - headRy * 0.42, headRx * 0.2, headCy - headRy * 0.95);
            ctx.fill();
            ctx.stroke();
            break;
        }
...
const drawMark = (ctx, accentColor, unit, frame, animation, expression, accessory, hairColor) => {
    ...
    for (const spec of armSpecs) {
        ...
        drawArmStroke(ctx, attachX, attachY, midX, midY, endX, endY, lineWidth, outlineWidth, accentColor, alpha);
    }
    ctx.globalAlpha = 1;
    ctx.fillStyle = accentColor;
    ctx.beginPath();
    ...
    const drawEyePair = (ryScale, rxScale) => {
...
const resolveCanvasResolutionScale = (graphScale) => {
...
export const MascotSprite = ({ animation = "sway", expression = "normal", accessory = "none", hairColor, scale, size = DEFAULT_SIZE, speedMs = 300, className, color, testId, graphScale, }) => {
    ...
    return (_jsx("canvas", { ref: canvasRef, className: className, width: Math.max(1, Math.round(canvasSize * resolutionScale)), height: Math.max(1, Math.round(canvasSize * resolutionScale)), style: {
            width: `${canvasSize}px`,
