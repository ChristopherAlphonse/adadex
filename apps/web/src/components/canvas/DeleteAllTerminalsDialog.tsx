  ...
  const handleConfirm = useCallback(async () => {
    ...
    for (const terminal of activeTargets) {
      try {
        ...
        if (!response.ok) {
          failures.push(
            `${terminal.coordinationName || terminal.label || terminal.terminalId}: ${await readDeleteFailureMessage(
              response,
        ...
        failures.push(
          `${terminal.coordinationName || terminal.label || terminal.terminalId}: ${
