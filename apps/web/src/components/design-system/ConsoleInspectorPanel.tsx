...
export const ConsoleInspectorPanel = ({
  ...
  const workspaceLabel =
    terminal?.workspaceMode === "worktree"
      ? `worktree · ${terminal.coordinationId}`
      : terminal?.workspaceMode === "shared"
        ? `shared · ${terminal.coordinationId}`
        : selectedNode?.workspaceMode === "worktree"
          ? `worktree · ${selectedNode.coordinationId}`
          : selectedNode
            ? `shared · ${selectedNode.coordinationId}`
            : "—";
          ...
          <>
            <Section title="Metadata">
              <div className="space-y-1">
                <KV label="Orchestration" value={selectedNode.coordinationId} mono />
                <KV label="Workspace" value={workspaceLabel} mono />
                <KV
                  label="Session"
                  value={selectedNode.sessionId ?? terminal?.terminalId ?? "—"}
                  mono
                />
