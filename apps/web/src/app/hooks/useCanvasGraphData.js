...
const isDeckLeadCoordinationId = (id) => id === DECK_LEAD_ID || id === LEGACY_DECK_LEAD_ID;
...
const orchestrationColor = (coordinationId, deckColor) => deckColor && deckColor.length > 0
    ? deckColor
    : MASCOT_COLORS[hashString(coordinationId) % MASCOT_COLORS.length];
...
const normalizeDeckCoordinationSummary = (value) => {
    ...
    const scopeRecord = record.scope !== null && typeof record.scope === "object"
        ? record.scope
        : null;
    const appearanceRecord = record.mascot !== null && typeof record.mascot === "object"
        ? record.mascot
    ...
    return {
        ...
        displayName: typeof record.displayName === "string" ? record.displayName : record.coordinationId,
        description: typeof record.description === "string" ? record.description : "",
        status,
        color: typeof record.color === "string" ? record.color : null,
        mascot: {
            animation: typeof appearanceRecord?.animation === "string" ? appearanceRecord.animation : null,
            expression: typeof appearanceRecord?.expression === "string" ? appearanceRecord.expression : null,
            accessory: typeof appearanceRecord?.accessory === "string" ? appearanceRecord.accessory : null,
            hairColor: typeof appearanceRecord?.hairColor === "string" ? appearanceRecord.hairColor : null,
        },
        scope: {
...
export const useCanvasGraphData = ({ columns, enabled, agentRuntimeStates, }) => {
    const [deckOrchestrations, setDeckOrchestrations] = useState([]);
    ...
    const sessionsByOrchestrationId = new Map();
    ...
    const prevBoss = prevNodes.get(DECK_LEAD_NODE_ID) ?? prevNodes.get(LEGACY_DECK_LEAD_NODE_ID) ?? null;
    ...
    for (const terminal of columns) {
        if (!isDeckLeadCoordinationId(terminal.coordinationId))
            continue;
    ...
    for (const session of inactiveSessions) {
        if (!session.coordinationId || !seenOrchestrationIds.has(session.coordinationId))
            continue;
