...
vi.mock("../src/components/canvas/SessionNode", () => ({
    SessionNode: ({ node, onClick, }) => (_jsx("button", { type: "button", "data-node-id": node.id, onClick: () => onClick(node.id), children: node.label })),
}));
vi.mock("../src/components/canvas/OctopusNode", () => ({
    OctopusNode: ({ node, onClick, }) => (_jsxs("g", { "data-node-id": node.id, onClick: () => onClick(node.id), onKeyDown: (event) => {
            if (event.key === "Enter" || event.key === " ") {
                onClick(node.id);
            }
        }, children: [_jsx("circle", { cx: node.x, cy: node.y, r: node.radius }), _jsx("title", { children: node.label })] })),
}));
vi.mock("../src/components/canvas/CanvasTerminalColumn", () => ({
    CanvasTerminalColumn: ({ node, panelRef, onMinimize, onClose, onFocus, }) => (_jsxs("section", { ref: panelRef, "data-testid": `panel-${node.id}`, tabIndex: -1, onFocusCapture: onFocus, onPointerDown: onFocus, children: ["panel ", node.id, " label ", node.label, _jsx("button", { type: "button", onClick: onMinimize, children: "Minimize terminal panel" }), _jsx("button", { type: "button", onClick: onClose, children: "Close terminal session" })] })),
}));
...
describe("CanvasPrimaryView", () => {
    ...
    it("minimizes a terminal panel separately from shutting down the terminal session", async () => {
        ...
        render(_jsx(CanvasPrimaryView, { columns: [
                {
            ...
            ], isUiStateHydrated: true, onCloseActiveSession: onCloseActiveSession }));
        const [terminalButton] = screen.getAllByRole("button", { name: "terminal-1" });
        expect(terminalButton).toBeDefined();
        if (!terminalButton)
            throw new Error("Missing terminal button");
    ...
    it("auto-opens a newly created child terminal when its parent panel is already open", async () => {
        const { rerender } = render(_jsx(CanvasPrimaryView, { columns: [
                {
                    terminalId: "terminal-1",
                    label: "terminal-1",
                    state: "live",
        ...
        rerender(_jsx(CanvasPrimaryView, { columns: [
                {
                    terminalId: "terminal-1",
                    label: "terminal-1",
                    state: "live",
            ...
            ], isUiStateHydrated: true, recentlyCreatedTerminal: {
                terminalId: "terminal-2",
                label: "terminal-2",
                state: "live",
                coordinationId: "orchestration-a",
    ...
    it("updates an open terminal panel label when the terminal is renamed", async () => {
        ...
        fireEvent.click(terminalButton);
        await waitFor(() => {
            expect(screen.getByTestId("panel-a:terminal-1")).toHaveTextContent("panel a:terminal-1 label orchestration-a");
        });
        rerender(_jsx(CanvasPrimaryView, { columns: [
                {
                    terminalId: "terminal-1",
        ...
        await waitFor(() => {
            expect(screen.getByTestId("panel-a:terminal-1")).toHaveTextContent("panel a:terminal-1 label renamed-orchestration");
        });
    });
    it("shows orchestration maintenance actions in the context menu and passes the orchestration ID", async () => {
        ...
        const { container } = render(_jsx(CanvasPrimaryView, { columns: [], isUiStateHydrated: true, onOrchestrationAction: onOrchestrationAction }));
