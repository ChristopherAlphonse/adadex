...
const initializeProject = (workspaceCwd: string, preferredName?: string) => {
  const projectName = preferredName?.trim() || basename(workspaceCwd) || "adadex-project";
...
const resolveStartupProjectContext = (workspaceCwd: string) => {
  ...
  const projectDisplayName = basename(workspaceCwd) || "adadex-project";
...
const initProject = (name?: string) => {
  ...
  console.log(
    `${created ? "Initialized" : "Updated"} Adadex project "${projectConfig.displayName}" at ${projectPath}`,
  );
  console.log("  .adadex/ directory ready (project metadata, coordinations, worktrees)");
  console.log(`  Global state: ${projectStateDir}`);
  console.log("  .gitignore updated");
  console.log("\nRun `adadex` to start the dashboard.");
};
...
const apiError = () => {
  console.error(
    `Error: Could not reach API at ${resolveRuntimeApiBase()}. Start Adadex in this project first.`,
  );
  process.exit(1);
};
...
const coordinationCreate = async () => {
  const name = args[2];
  if (!name || name.startsWith("-")) {
    console.error("Error: coordination name is required.");
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`Created coordination "${String(data.coordinationId ?? "")}"`);
  } catch {
    apiError();
  }
};

const coordinationList = async () => {
  const apiBase = resolveRuntimeApiBase();

  try {
    const response = await fetch(`${apiBase}/api/deck/coordinations`);
    if (!response.ok) {
      console.error("Error: failed to fetch coordinations.");
      process.exit(1);
    }

    const coordinations = (await response.json()) as Array<Record<string, unknown>>;
    ...
    for (const coordination of coordinations) {
      const description = coordination.description ? ` — ${coordination.description}` : "";
      console.log(`  ${coordination.coordinationId}${description}`);
    }
  } catch {
    apiError();
  }
...
const terminalCreate = async () => {
  ...
  if (coordinationId) body.coordinationId = coordinationId;
...
const terminalList = async () => {
  ...
  try {
    ...
    for (const terminal of terminals) {
      ...
      const name = String(terminal.coordinationName ?? terminal.label ?? terminalId);
...
const main = async () => {
  ...
  if (command === "projects" || command === "project") {
    ...
    if (projects.length === 0) {
      console.log(
        "No projects registered yet. Run `adadex` or `adadex init` in a project directory.",
      );
      return;
    }
  ...
  console.log(`Usage:
  adadex                               Start the dashboard in the current project
  adadex init [project-name]           Initialize the current directory explicitly
  adadex projects                      List registered projects

  adadex coordination create <name>    Create a coordination (Adadex must be running)
  adadex coordination list             List coordinations
  adadex terminal create [options]     Create a terminal
    ...
    --coordination-id                  Existing coordination ID to attach to
  ...
  adadex terminal list                 List terminal lifecycle state
  adadex terminal stop <id>            Stop a terminal session
  adadex terminal kill <id>            Kill a terminal session or recorded process
  adadex terminal prune                Remove stale, stopped, and exited terminal records
  adadex channel send <id> <msg>       Send a channel message
  adadex channel list <id>             List channel messages`);
  process.exit(1);
};
