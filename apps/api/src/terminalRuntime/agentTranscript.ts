...
const extractTextContent = (msg: AgentTranscriptMessage): string | null => {
...
const getMessageRole = (msg: AgentTranscriptMessage): string | null =>
  msg.message?.role ?? msg.role ?? null;

const isUserTextMessage = (msg: AgentTranscriptMessage): boolean => {
  if (msg.type !== "user") {
    return false;
  }
...
const isAssistantTextMessage = (msg: AgentTranscriptMessage): boolean => {
...
const isToolUseMessage = (msg: AgentTranscriptMessage): boolean => {
  if (msg.type !== "assistant") {
    return false;
  }
...
const extractToolUseSummary = (msg: AgentTranscriptMessage): string | null => {
  const content = msg.message?.content ?? msg.content;
  if (!Array.isArray(content)) {
    return null;
  }
...
export const parseAgentTranscript = (transcriptPath: string): ConversationTurn[] | null => {
  ...
  for (const line of rawJsonl.split(/\r?\n/)) {
    ...
    try {
      ...
      if (parsed !== null && typeof parsed === "object" && !Array.isArray(parsed)) {
        const msg = parsed as AgentTranscriptMessage;
