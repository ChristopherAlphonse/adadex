...
const resolveRepoPromptsDir = (): string => {
  const fromTestFile = join(dirname(fileURLToPath(import.meta.url)), "..", "..", "..", "prompts");
  const candidates = [
  ...
  for (const dir of candidates) {
    if (existsSync(join(dir, "coordination-context-init.md"))) {
      return dir;
    }
  }
  return fromTestFile;
...
class FakeGitClient implements GitClient {
  ...
  createPullRequest({
    ...
    const pullRequest = {
      number: nextNumber,
      url: `https://github.com/hesamsheikh/adadex/pull/${nextNumber}`,
      title,
      baseRef,
      headRef,
      state: "OPEN" as const,
...
describe("createApiServer", () => {
    ...
    while (Date.now() < timeoutAt) {
      if (existsSync(registryPath)) {
        try {
          ...
          const document = JSON.parse(raw) as TDocument;
          if (predicate(document)) {
            return document;
          }
        } catch {
          // Empty or partially written registry file; retry shortly.
        }
      }
  ...
  const writeConversationTranscript = (
    ...
    const transcriptDirectory = join(workspaceCwd, ".adadex", "state", "transcripts");
  ...
  const writeAgentTurns = (
    ...
    const turnsPath = join(
      transcriptDirectory,
      `${encodeURIComponent(sessionId)}.agent-turns.json`,
    );
  ...
  it("returns session summaries for GET /api/conversations", async () => {
    ...
    writeAgentTurns(workspaceCwd, "terminal-1", [
      {
        turnId: "turn-1",
        role: "user",
    ...
    writeConversationTranscript(workspaceCwd, "terminal-2-agent-1", [
      {
        ...
        coordinationId: "terminal-2",
        timestamp: "2026-03-05T11:00:00.000Z",
      },
    ]);
    writeAgentTurns(workspaceCwd, "terminal-2-agent-1", [
      {
        turnId: "turn-1",
        role: "user",
        content: "summarize",
    ...
    await expect(listResponse.json()).resolves.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          coordinationName: "Investigate flaky CI failures",
        }),
      ]),
    );
  });
          ...
          terminals: [
            {
              terminalId: "terminal-1",
              coordinationId: "terminal-1",
              coordinationName: "Adadex Terminal 1",
  ...
  it("reports file-backed workspace setup status and updates it through setup actions", async () => {
    ...
    expect(existsSync(join(workspaceCwd, ".adadex"))).toBe(false);
    ...
    expect(initialPayload.hasAnyCoordinations).toBe(false);
    expect(initialPayload.steps).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ id: "create-coordinations", complete: false }),
      ]),
    );
    ...
    expect(existsSync(join(workspaceCwd, ".adadex", "project.json"))).toBe(true);
    expect(existsSync(join(workspaceCwd, ".adadex", "coordinations"))).toBe(true);
    expect(existsSync(join(workspaceCwd, ".adadex", "worktrees"))).toBe(true);
    ...
    expect(finalPayload.hasAnyCoordinations).toBe(true);
    expect(finalPayload.coordinationCount).toBe(1);
    expect(finalPayload.steps).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ id: "create-coordinations", complete: true }),
      ]),
    );
  });
  ...
  it("creates tentacles with suggested skills and appends the managed context block", async () => {
    ...
    const response = await fetch(`${baseUrl}/api/deck/coordinations`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    const context = readFileSync(
      join(workspaceCwd, ".adadex", "coordinations", "docs", "CONTEXT.md"),
      "utf8",
    );
    ...
    const listResponse = await fetch(`${baseUrl}/api/deck/coordinations`, {
      headers: { Accept: "application/json" },
    });
    expect(listResponse.status).toBe(200);
    await expect(listResponse.json()).resolves.toEqual(
  ...
  it("updates tentacle suggested skills and removes the managed context block when cleared", async () => {
    ...
    const createResponse = await fetch(`${baseUrl}/api/deck/coordinations`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    const updateResponse = await fetch(`${baseUrl}/api/deck/coordinations/docs/skills`, {
      method: "PATCH",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    const contextPath = join(workspaceCwd, ".adadex", "coordinations", "docs", "CONTEXT.md");
    expect(readFileSync(contextPath, "utf8")).toContain("- `code-review-specialist`");

    const clearResponse = await fetch(`${baseUrl}/api/deck/coordinations/docs/skills`, {
      method: "PATCH",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    const registryDocument = await waitForRegistryDocument<{
      ...
      document.terminals.some(
        (terminal) =>
          ...
          terminal.coordinationId === "terminal-1" &&
  ...
  it("injects a default tentacle context prompt for tentacle terminals", async () => {
    ...
    const createResponse = await fetch(`${baseUrl}/api/terminals`, {
      ...
      body: JSON.stringify({ coordinationId: "docs", workspaceMode: "shared" }),
    });
    expect(createResponse.status).toBe(201);
    await expect(createResponse.json()).resolves.toEqual(
      expect.objectContaining({
  ...
  it("creates isolated worktree terminals with dedicated cwd", async () => {
    ...
    const expectedWorktreePath = join(workspaceCwd, ".adadex", "worktrees", "terminal-1");
    expect(gitClient.getWorktree(expectedWorktreePath)).toEqual(
      expect.objectContaining({
        cwd: workspaceCwd,
        branchName: "adadex/terminal-1",
        baseRef: "HEAD",
      }),
    );
  ...
  it("returns git status for worktree tentacles", async () => {
    ...
    const worktreePath = join(workspaceCwd, ".adadex", "worktrees", "terminal-1");
    gitClient.setWorktreeStatus(worktreePath, {
      branchName: "adadex/terminal-1",
      upstreamBranchName: "origin/adadex/terminal-1",
      isDirty: true,
      aheadCount: 2,
      behindCount: 1,
      insertedLineCount: 0,
    ...
    const statusResponse = await fetch(`${baseUrl}/api/coordinations/terminal-1/git/status`, {
      method: "GET",
      headers: {
        Accept: "application/json",
      },
  ...
  it("commits pending worktree changes with a required message", async () => {
    ...
    const commitResponse = await fetch(`${baseUrl}/api/coordinations/terminal-1/git/commit`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
  ...
  it("pushes worktree branch and updates ahead count", async () => {
    ...
    const pushResponse = await fetch(`${baseUrl}/api/coordinations/terminal-1/git/push`, {
      method: "POST",
      headers: {
        Accept: "application/json",
      },
  ...
  it("syncs worktree branch with base ref", async () => {
    ...
    const syncResponse = await fetch(`${baseUrl}/api/coordinations/terminal-1/git/sync`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
  ...
  it("returns PR status for worktree tentacles", async () => {
    ...
    gitClient.setWorktreePullRequest(worktreePath, {
      number: 142,
      url: "https://github.com/hesamsheikh/adadex/pull/142",
      title: "feat: worktree git lifecycle menu",
      baseRef: "main",
      headRef: "adadex/terminal-1",
      state: "OPEN",
      isDraft: false,
      mergeable: "MERGEABLE",
      mergeStateStatus: "CLEAN",
    ...
    const prStatusResponse = await fetch(`${baseUrl}/api/coordinations/terminal-1/git/pr`, {
      method: "GET",
      headers: {
        Accept: "application/json",
      },
  ...
  it("creates PR for worktree tentacles and returns PR snapshot", async () => {
    ...
    const createPrResponse = await fetch(`${baseUrl}/api/coordinations/terminal-1/git/pr`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    await expect(createPrResponse.json()).resolves.toEqual({
      ...
      status: "open",
      number: 101,
      url: "https://github.com/hesamsheikh/adadex/pull/101",
      title: "feat: expose worktree lifecycle actions",
      baseRef: "main",
      headRef: "adadex/terminal-1",
      isDraft: false,
  ...
  it("merges the current branch PR for worktree tentacles", async () => {
    ...
    gitClient.setWorktreePullRequest(worktreePath, {
      number: 190,
      url: "https://github.com/hesamsheikh/adadex/pull/190",
    ...
    const mergeResponse = await fetch(`${baseUrl}/api/coordinations/terminal-1/git/pr/merge`, {
      method: "POST",
      headers: {
        Accept: "application/json",
      },
  ...
  it("reads builtin prompts from the live promptsDir after server start", async () => {
    ...
    writeFileSync(join(promptsDir, "coordination-update.md"), "version two\n", "utf8");

    const response = await fetch(`${baseUrl}/api/prompts/coordination-update`, {
      method: "GET",
      headers: {
        Accept: "application/json",
      },
  ...
  it("spawns a shared-workspace todo agent for an individual item", async () => {
    ...
    writeFileSync(
      join(workspaceCwd, ".adadex", "coordinations", "docs-knowledge", "CONTEXT.md"),
      "# Docs & Knowledge\n",
      "utf8",
    );
    writeFileSync(
      join(workspaceCwd, ".adadex", "coordinations", "docs-knowledge", "todo.md"),
    ...
    const solveResponse = await fetch(
      `${baseUrl}/api/deck/coordinations/docs-knowledge/todo/solve`,
      {
        method: "POST",
        headers: {
          Accept: "application/json",
    ...
    await expect(listResponse.json()).resolves.toEqual([
      expect.objectContaining({
        terminalId: "docs-knowledge-todo-0",
        coordinationId: "docs-knowledge",
        coordinationName: "Docs & Knowledge",
        workspaceMode: "shared",
      }),
    ]);
  });
  ...
  it("limits swarm prompts to the top-priority items that fit under the child cap", async () => {
    ...
    const swarmResponse = await fetch(`${baseUrl}/api/deck/coordinations/docs-knowledge/swarm`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
