...
Adadex uses child terminals to split work into parallel streams.
...
Deck creates child agents from todo items by resolving prompt templates. The prompt receives the coordination name, coordination ID, path to `.adadex/coordinations/<coordination-id>/`, todo text, terminal ID, API port, workspace guidance, and parent terminal ID when a parent exists.
...
- each task fits one coordination or one todo item
...
1. create or pick a coordination
...
In worktree mode, each worker gets a branch named `adadex/<worker-terminal-id>` under `.adadex/worktrees/<worker-terminal-id>/` and is told to commit its work. The parent coordinator is responsible for merging branches, running tests, and updating coordination state.

## Parent coordinator behavior

When a swarm has more than one target item, Adadex creates a parent terminal like `<coordination-id>-swarm-parent`. The parent prompt contains:

- the list of worker terminal IDs and assigned todo indices
- commands for creating each worker terminal
- communication instructions for `adadex channel send`
- a completion strategy for shared mode or worktree mode
- the final requirement to review, test, and update coordination docs/todos
...
Each parent can have up to 9 child terminals. If a swarm has more incomplete todo items than that, Adadex uses todo order as priority order and defers the overflow.

Worker terminal IDs are derived from the coordination ID and todo index. That makes duplicate detection simple: Adadex refuses to start a second active solve or swarm for the same item pattern.
